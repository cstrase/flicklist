...
var api = {
  ...
  token: "543342f02785f67a3d5fdd709af26588",
...
function render() {
  ...
  model.watchlistItems.forEach(function(movie) {
    ...
    var button = $('<button class="btn btn-danger">Add me!</button');
    button.click(function() {
      var idx = model.watchlistItems.indexOf(movie);
      model.watchlistItems = model.watchlistItems.splice(idx, 1);
      render();
    });
    //var watchedButton = $('<button></button>').text("I watched it");
    //title.append(watchedButton);

    //watchedButton.click(function(movie) {
    //  if (model.watchlistItems.indexOf(movie) !== -1) {
      //  model.watchlistItems.splice(indexOf(movie), 1);
    //  }
      //title.prop("remove", model.watchlistItems.indexOf(movie) !== -1)
    //  render();
